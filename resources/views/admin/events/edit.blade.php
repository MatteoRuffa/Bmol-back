...
<section class="container py-5">
    <div class="container rounded-2 container-table">
        <h1 class=" fw-bolder">Edit a Events:</h1>
        <h3>General informations</h3>
        <div id="ls-edit" class="">
        <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" id="">
            @csrf
            @method('PUT')
                <div class="mb-3 @error('name') is-invalid @enderror">
                    <label for="name" class="form-label fs-5 fw-medium">Name</label>
                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                        id="name" name="name" value="{{ old('name', $event->name) }}"  maxlength="255" minlength="3">
                ...
                <div class="row">
                    <div class="mb-3 col @error('hour') @enderror">
                        ...
                        <input class="form-control @error('hour') is-invalid @enderror"
                            id="hour" name="hour" value="{{ old('hour',  $event->hour) }}" type="time" step="3600" required>
                    ...
                    <div class="mb-3 col @error('date') @enderror">
                        ...
                        <input class="form-control @error('date') is-invalid @enderror" id="date" name="date" value="{{ old('date',  $event->date) }}" type="date" required>
                ...
                <div class="address d-flex justify-content-center flex-column">
                    <div class="mb-3">
                        <div class="form-check">
                            <input class="form-check-input mt-2"  type="checkbox" id="visibility" name="visibility" value="1" {{ old('visibility',  $event->visibility) ? 'checked' : ''}}>
                    ...
                    <div class="w-50">
                        ...
                        <div class="w-75 mb-3">
                            ...
                            <input type="file" accept="image/*" class="form-control upload_image" id="uploadImage" name="image_cover"
                                value="{{ old('image_cover',  $event->image_cover) }}">
                            @error('image_cover')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="text-center mx-auto justify-content-center d-flex gap-2">
                    <button type="submit" class="btn-2 draw-border-2 p-2 px-3 mt-3 mx-3"><i class="fa-solid fa-plus"></i> Edit the event</button>
